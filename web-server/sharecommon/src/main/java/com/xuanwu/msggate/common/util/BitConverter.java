...
public class BitConverter {
	...
	public static int toInt32(byte[] bufer) {
		return (((bufer[3] & 0xff) << 24) | ((bufer[2] & 0xff) << 16)
				| ((bufer[1] & 0xff) << 8) | ((bufer[0] & 0xff) << 0));

	}
	
	public static String toStr(byte[] bufer) {
		return new String(bufer);
	}

	public static long toInt64(byte[] bufer) {
		return ((((long) bufer[7] & 0xff) << 56)
				| (((long) bufer[6] & 0xff) << 48)
				| (((long) bufer[5] & 0xff) << 40)
				| (((long) bufer[4] & 0xff) << 32)
				| (((long) bufer[3] & 0xff) << 24)
				| (((long) bufer[2] & 0xff) << 16)
				| (((long) bufer[1] & 0xff) << 8) | (((long) bufer[0] & 0xff) << 0));
	}
